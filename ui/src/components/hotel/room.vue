<template>
  <el-card class="box-card">
    ...
    <m-bread level1="酒店管理" level2="房间管理"></m-bread>
    <!--搜索-->
    ...
    <el-table :data="tableData"style="width: 100%" class="Eltable">
      ...
      <el-table-column   prop="roomNo"  label="房间数量"></el-table-column>
      ...
      <el-table-column   prop="" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="open(scope.row.roomId)" icon="el-icon-edit"></el-button>
          <el-button size="mini" type="danger" @click="remove(scope.row.roomId)" icon="el-icon-delete"></el-button>
    ...
    <el-dialog  :title="title" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="form">
        ...
        <el-form-item label="房间定价" label-width="80px">
          <el-input v-model="form.roomPrice" autocomplete="off"></el-input>
        </el-form-item>

        <el-form-item label="房间数量" label-width="80px">
        ...
        methods: {
            ...
            remove(id){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    this.$http.delete('/room/remove/'+id).then(res=>{
          ...
          /*添加用户*/
            async  save(){
              ...
              const res = await this.$http.post('/room/save',this.form);
              const {msg,code,data} =res.data;
                if(code ==='ok'){
                    this.getList()
                    this.form={}
            ...
            role(obj){
                ...
                this.$http.get('/room/select').then(res=>{
            ...
            setRole(){
                this.$http.get('/room/setrole/'+this.currnadminId+'/'+this.form.adminRole).then(res=>{
                    const {msg,data,code}=res.data
                });
          ...
          queryList(){
            ...
            async getList(){
                ...
                const res = await this.$http.post('/room/list',this.page);
                const {msg,code,data} =res.data;
                var obj = data[0];
                console.log(obj)
                if(code ==='ok'){
